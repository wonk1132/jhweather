...
/** For this assignment, I'm using a single file to express all of the various bits (except for
  * tests). This is generally not best-practice (almost every type should have its own file), but I
  * think for this particular task, it is easier to read and follow if it's all just in-lime.
  *
  ...
  * that the pain of the tagless final abstraction is more pain than benefit.
  */
...
case class Forecast(
  characterization: Forecast.ForecastCharacterization,
...
object Forecast:
  ...
  def fromNws(nws: NwsForecast): Either[Throwable, Forecast] =
    for
      ...
      characterization <- Either.catchNonFatal(
        ForecastCharacterization.fromTemperatureF(perioedOne.temperature)
      )
    yield Forecast(characterization, perioedOne.shortForecast)

  def fromJsonString(str: String): Either[Throwable, Forecast] = decode[Forecast](str).leftMap { error =>
  ...
  given Decoder[ForecastCharacterization] = new Decoder[ForecastCharacterization]:
    override def apply(c: HCursor): Result[ForecastCharacterization] =
      for
        f <- c.value.as[String]
        fc <- ForecastCharacterization.fromString(f).leftMap(e => DecodingFailure(e.getMessage, c.history))
      yield fc
...
object WeatherRoutes:
  ...
  def apply[F[_]: Async](
    ...
    HttpRoutes
      .of[F] {
        case _ @GET -> Root / "forecast" / "short" / "lat" / LatitudeVar(
              latitude
            ) / "lon" / LongitudeVar(longitude) =>
          (for
