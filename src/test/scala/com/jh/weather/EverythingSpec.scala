...
class EverythingSpec extends AnyFunSuite:
  ...
  given Metrics[IO] = Metrics.apply[IO](config)
  ...
  test("deserialiation of nws point json succeeds") {
    ...
    NwsPoint
      ...
      .bimap(
        error => throw error,
        point => point.properties.forecast.mustBe("https://api.weather.gov/gridpoints/TOP/32,81/forecast")
      )
  }

  test("deser of nws forecast json succeeds") {
    NwsForecast
      .fromJson(forecastJson)
      .bimap(
        error => throw error,
        forecast =>
  ...
  test("NWS forecast results in correct JH Forecast") {
    val nwsForecast = NwsForecast.fromJson(forecastJson).getOrElse(throw Throwable("forecast deser error"))
    Forecast
      .fromNws(nwsForecast)
      .bimap(
        error => throw error,
  ...
  test("IT test: pull forecast from NWS succeeds") {
    (for
      c <- client.attemptT
      forecast: Forecast <- EitherT(c.fetchForecast(
        Latitude(39.7456),
        Longitude(-97.0892)
      ))
